...
/**
 * The complete YC portfolio as one static JSON array, published by yc-oss on
 * GitHub Pages. Preferred over YC_API_URL: 6,204 companies against the API's
 * 1,425, in one request, with `isHiring` / `team_size` / `stage` fields the
 * API does not return. See F-416.
 */
const YC_MIRROR_URL = 'https://yc-oss.github.io/api/companies/all.json';
 ...
 * @property {string[]} [tags]          Portfolio-assigned tags.
 * @property {string[]} [industries]    Industry / sub-industry labels.
 * @property {string[]} [regions]       Geographic labels.
 * @property {number}   [teamSize]      Headcount, when published.
 * @property {string}   [status]        'Active' | 'Acquired' | 'Inactive' | 'Public'.
 * @property {string}   [oneLiner]      One-sentence description.
 * @property {string}   [stage]         Funding stage label, when published.
 * @property {boolean}  [isHiring]      Whether the company says it is hiring (yc-oss only).
 */
...
export function parseYCPayload(payload) {
  ...
  for (const item of list) {
    ...
    // Carry YC's own classification through. Without it every consumer has to
    // treat 5000 portfolio companies as one undifferentiated list, when the
    // only ones worth probing for a given search are a narrow slice of them
    // (see jarvis/discover-ats.mjs --seeds, which filters on tags/industries).
    // Purely additive — existing consumers ignore these.
    // Two payload shapes reach here. The v0.1 API uses camelCase and nests
    // everything under `companies`; the yc-oss mirror is a bare array in
    // snake_case with two extra classification fields (`industry`,
    // `subindustry`) and the one field neither the API nor a portfolio page
    // gives you: whether the company says it is hiring right now. Both are
    // read, so the consumer never has to know which source it got.
    const strList = (v) => (Array.isArray(v) ? v.filter(x => typeof x === 'string' && x.trim()).map(x => x.trim()) : []);
    const str = (...vs) => { for (const v of vs) if (typeof v === 'string' && v.trim()) return v.trim(); return ''; };
    const tags = strList(item.tags);
    const industries = [...new Set([
      ...strList(item.industries),
      ...[item.industry, item.subindustry].filter(v => typeof v === 'string' && v.trim()).map(v => v.trim()),
    ])];
    const regions = strList(item.regions);
    if (tags.length) entry.tags = tags;
    if (industries.length) entry.industries = industries;
    if (regions.length) entry.regions = regions;
    const teamSize = Number.isFinite(item.teamSize) ? item.teamSize : item.team_size;
    if (Number.isFinite(teamSize)) entry.teamSize = Number(teamSize);
    const status = str(item.status);
    if (status) entry.status = status;
    const oneLiner = str(item.oneLiner, item.one_liner);
    if (oneLiner) entry.oneLiner = oneLiner;
    const stage = str(item.stage);
    if (stage) entry.stage = stage;
    if (typeof item.isHiring === 'boolean') entry.isHiring = item.isHiring;
 ...
 */
// The YC API serves a fixed 25 companies per page and IGNORES `per_page`,
// advertising the real span in `totalPages` (~246, i.e. ~6000 companies).
// The original loop asked for per_page=1000 and stopped when a page returned
// fewer than 1000 rows — which is every page — so it always broke after the
// first one and returned 25 companies, no matter what `maxPages` said. The
// default below reflects the original intent (a few thousand companies), and
// the loop now trusts `totalPages` and an empty page as its stop conditions.
export async function fetchYCCompanies({ timeoutMs = DEFAULT_TIMEOUT_MS, maxPages = 120 } = {}) {
  /** @type {SeedCompany[]} */
  const all = [];
  const seen = new Set();

  // THE MIRROR FIRST, AND WHY (F-416). `api.ycombinator.com/v0.1/companies`
  // paginates to 1,425 companies. The portfolio is 6,204. Whatever the API is
  // actually scoped to, it is not the portfolio, and for six months the only
  // automatic source of startup names in this project was reading 23% of it.
  // yc-oss publishes the whole thing as one static JSON file on GitHub Pages —
  // one request, no pagination, and it carries `isHiring`, `team_size` and
  // `stage`, none of which the API returns. The API stays as the fallback, so
  // a dead mirror costs coverage rather than the source.
  try {
    const res = await fetchWithTimeout(YC_MIRROR_URL, { timeoutMs: Math.max(timeoutMs, 60_000) });
    const entries = parseYCPayload(await res.json());
    if (entries.length > 1_000) return entries;
  } catch {
...
export async function fetchA16zCompanies({ timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  ...
  const rows = parseA16zPayload(html);

  // ZERO IS NOT AN ANSWER (F-417). All three parse strategies below are
  // heuristics against a marketing page that a16z can restyle whenever it
  // likes, and on 2026-09-08 all three stopped matching: the fetch returned
  // 512 KB of HTML and the parser returned an empty list. Nothing was thrown,
  // so `--seeds a16z` reported "0 companies after filtering" — indistinguishable
  // from a firm that has no hardware portfolio. A source that cannot find a
  // single company in a page this size is broken, and has to say so.
  if (!rows.length && html.length > 10_000) {
    throw new Error(
      `vc-portfolios: a16z portfolio parsed to 0 companies from ${html.length} bytes — ` +
      `the page layout has changed and parseA16zPayload needs updating`);
  }
  return rows;
}
...
export const SEED_SOURCES = {
  ...
  // Not a portfolio and not a fetch — a curated list, for the reason written
  // at the top of that file: the hardware-focused funds publish no
  // machine-readable portfolio, and ten bespoke HTML parsers are a worse trade
  // than writing the names down. Registered here so `--seeds hardware` goes
  // through exactly the same filter, dedupe and live-verification path as YC.
  hardware: {
    fetch: async (opts) => {
      const mod = await import('./hardware-startups.mjs');
      return mod.listHardwareStartups(opts);
    },
    label: 'Curated hardware / deeptech list',
  },
};
