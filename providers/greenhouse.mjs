...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    return jobs.filter(/** @param {any} j */ j => j.absolute_url).map(/** @param {any} j */ j => ({
      ...
      // WHERE THE DESCRIPTION LIVES, because the posting URL does not always
      // say (F-404). A company that hosts its own Greenhouse board publishes
      // `agilityrobotics.com/about/job-post?gh_jid=5986750004` — the board slug
      // the detail endpoint needs appears nowhere in it, so the enricher could
      // never read those postings and they sat in the deck with no description,
      // no visa check and a title-only fit score. This board's own API URL is
      // known right here; the posting id turns it into the detail endpoint.
      detailApi: j.id ? `${apiUrl.replace(/\/jobs\/?$/, '')}/jobs/${j.id}` : null,
    }));
  },
};
