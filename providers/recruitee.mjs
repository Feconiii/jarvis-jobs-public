...
export function parseRecruiteeResponse(json, companyName) {
  ...
  return offers.map(j => {
    ...
    // THE DESCRIPTION WAS ALREADY IN HAND AND WAS BEING THROWN AWAY (F-436).
    //
    // `/api/offers/` returns the full posting body — measured at 2,548
    // characters on a sampled EasyPost offer — and this mapping rebuilt each
    // job from an explicit field list that did not include it. So every
    // Recruitee posting reached the store with no description, nothing read
    // its work-authorisation language, and its fit was scored from the title
    // alone. There is no detail endpoint to add and no extra request to spend;
    // the text was in the response the scanner had already paid for.
    //
    // `requirements` is joined in for the same reason SmartRecruiters joins its
    // sections: "must be authorized to work in the US without sponsorship"
    // lands there at least as often as in the description proper, and missing
    // it is exactly the hard-block enrichment exists to catch.
    const description = [j.description, j.requirements]
    ...
    return {
      ...
      company: companyName,
      description,
      postedAt: typeof j.published_at === 'string' ? j.published_at : null,
    };
  });
}
