 ...
 * - url: the public posting URL is
 *   `jobs.smartrecruiters.com/<slug>/<id>-<title-slug>`.
 *
 *   `j.ref` is the API's OWN url - api.smartrecruiters.com/v1/companies/<slug>/
 *   postings/<id> - and this used to build the public URL by swapping the host,
 *   producing jobs.smartrecruiters.com/<slug>/postings/<id>. SmartRecruiters
 *   answers that with a 404 FOR LIVE POSTINGS. Verified against their API on
 *   the same requisition: /postings/<id> -> 404, /<id>-<title-slug> -> 200.
 *
 *   So `ref` is used only for what it is reliable for - the company slug, in
 *   the casing SmartRecruiters expects - and the public URL is assembled.
 *
 * @param {any} json
  ...
  return items.map(j => {
    ...
    if (typeof j.ref === 'string') {
      ...
      if (parsedRef
        ...
        const m = parsedRef.pathname.slice('/v1/companies/'.length)
          .match(/^([^/]+)[/]postings[/]([^/]+)$/);
        // The slug from `ref` keeps SmartRecruiters' own casing ("Intuitive"),
        // which the lowercased fallback below cannot reproduce.
        if (m) { slug = m[1]; id = id || m[2]; }
      }
    }
    if (!slug) slug = (companyName || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    if (slug && id) {
      url = `https://jobs.smartrecruiters.com/${slug}/${id}${slugified ? `-${slugified}` : ''}`;
    }
