  ...
  return items
    .map(item => {
      ...
      return {
        ...
        // `hiring_organization` wins by default because one tenant can host
        // several hiring orgs. But it is the API's LEGAL name, and everything
        // downstream — watchlist, hidden companies, --company, the dashboard
        // search — keys on the curated name in portals.yml. AMD returns
        // "Advanced Micro Devices, Inc", so searching "AMD" found nothing.
        // `use_entry_name: true` on the entry says this tenant is one company
        // and the configured name is the right one.
        company: String(
          (entry.use_entry_name ? entry.name : null) || d.hiring_organization || entry.name || '',
        ).trim(),
        location: d.full_location || [d.city, d.country].filter(Boolean).join(', '),
        // THE LIST API ALREADY CARRIES THE FULL DESCRIPTION, and this dropped
        // it on the floor. 91 AMD postings sat in the deck unreadable — no
        // skills judged, no visa language read, no degree requirement seen —
        // while the text was in the response that created them. There is no
        // detail endpoint to add and no extra request to make; the field was
        // simply never copied out. Same shape as Lever, which ships bodies in
        // its list API for the same reason.
        description: String(d.description || d.job_description || '').trim(),
        team: String(d.department || '').trim(),
      };
    })
    .filter(Boolean);
}
